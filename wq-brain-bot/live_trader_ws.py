...
def process_bar(symbol: str, bar_data: dict):
    ...
    n_bars = len(sym_bars)
    orb_min = strategy_module.PARAMS["orb_minutes"]

    if n_bars < orb_min + 5:
        logger.debug(f"  {symbol}: only {n_bars}/{orb_min + 5} bars — not enough data")
        return
...
async def main_async():
    ...
    for sym in symbols:
        ...
        if sym_bars.empty or len(sym_bars) < strategy_module.PARAMS["orb_minutes"] + 5:
            continue
        entries, exits = strategy_module.generate_signals(sym_bars)
